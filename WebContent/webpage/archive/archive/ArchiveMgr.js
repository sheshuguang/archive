	...
	$(function(){
          $('#archivetree').tree({
	          ...
	          onBeforeExpand:function(node,param){
        	  		$('#archivetree').tree('options').url = "loadTreeData.action?parentid=" + node.id;                       
	      	  },
    ...
    function uploadFiles() {
        ...
        if (!isLoadUploadify) {
            $("#uploadify").uploadify({//初始化函数
                ...
                'auto':false, //是否自动上传
                'multi':true,
                //                        'simUploadLimit':10, //同时上传文件的数量，设置了这个参数后，那么你会因设置multi:true和queueSizeLimit：8而可以多选8个文件，但如果一旦你将simUploadLimit也设置了，那么只会上传这个参数指定的文件个数，其它就上传不了
...
//                    uploadifyCancel('NFJSHS');
                    $("p[name='uploadfiles']").text(data.fileCount + " 文件等待上传");
                },
                'onSelectOnce':function (event, data) {
                    //                            alert(data.fileCount);
                    $("p[name='selectfiles']").text("共选择 " + data.fileCount + " 文件。");
                ...
                'onAllComplete':function (event, data) {
                    $("p[name='uploadfiles']").text(data.filesUploaded + " 文件成功上传");
